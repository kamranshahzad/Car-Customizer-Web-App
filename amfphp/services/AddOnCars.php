...
class AddOnCars {
	...
	function insertParts($vehicle_id , $part_category_id , $effected,  $part_name, $part_manufacturer,$description,$sku,$status,$part_icon,$part_price, $montlyPrice, $FrontView_asset ,$RearView_asset ,$InteriorView_asset ,$OtherView_asset , $FVIEW_asset , $RVIEW_asset , $IVIEW_asset , $FrontView_coord ,$RearView_coord ,$InteriorView_coord ,$OtherView_coord , $FVIEW_coord , $RVIEW_coord , $IVIEW_coord ){
		 ...
		 $this->insertPart_Proce($last_id , 'p' , $part_name , $sku , $description , $part_price ,  $montlyPrice );
		 return $last_id;
	}
   ...
   /* part_sets  */
	...
	function insertWheels($vehicle_id , $part_category_id , $effected,  $part_name, $part_manufacturer,$description,$sku,$status,$part_icon,$part_price, $montlyPrice , $FrontView_asset ,$RearView_asset ,$InteriorView_asset ,$OtherView_asset , $FVIEW_asset , $RVIEW_asset , $IVIEW_asset , $FrontView_coord ,$RearView_coord ,$InteriorView_coord ,$OtherView_coord , $FVIEW_coord , $RVIEW_coord , $IVIEW_coord ){
		 ...
		 $this->insertPart_Proce($last_id , 'w' , $part_name , $sku , $description , $part_price ,  $montlyPrice );
		 return $last_id;
	}
	...
	function getAllPartManufacturer(){
		return mysql_query("SELECT manufacturer_id,manufacturer_name FROM part_manufacturer");	
	}
	...
	function getAllPartTypes(){
		return mysql_query("SELECT part_type_id,part_type_name FROM part_types");	
	}
	...
	function getWheelTypesById($wid , $vehicle_id ){
		return mysql_query("SELECT wheel_types.wheel_type_id,wheel_types.wheel_type_name FROM wheel_types INNER JOIN wheels ON wheel_types.wheel_type_id = wheels.wheel_type_id WHERE wheels.vehicle_id='$vehicle_id' AND wheel_types.part_category_id ='$wid' GROUP BY wheel_types.wheel_type_id");	
	}
	function getWheelTypesById2($wid ){
		return mysql_query("SELECT wheel_type_id,wheel_type_name FROM wheel_types WHERE part_category_id ='$wid' GROUP BY wheel_type_id");	
	}
	...
	function getAllMakes(){
		return mysql_query("SELECT make_id, make_name,make_icon FROM makes");	
	}
	...
	*/
	...
	function getPartsCategories_easyWorker( $vehicle_id , $tabid ){
		return mysql_query("SELECT part_categories.part_category_id, part_categories.part_category_name FROM part_categories INNER JOIN parts ON part_categories.part_category_id  = parts.part_category_id WHERE parts.vehicle_id='$vehicle_id' AND part_categories.part_type_id='$tabid' GROUP BY parts.part_category_id");	
	}
	
	function getPartsLst_easyWorker($categoryId , $vehicle_id){
		return mysql_query("SELECT part_id,part_name FROM parts WHERE part_category_id='$categoryId' AND vehicle_id='$vehicle_id'");
	}
		
	function getWheelsCategories($vehicle_id){
		
	}
	
	function getWheelsLst_easyWorker($wheelSizeId){
	...
	*/
	...
	function insertPart_Proce( $PartId , $PartType , $PartLbl , $SKU , $Des , $Price1 , $Price2 ){
		return mysql_query("CALL CreateAccessory('$PartId', '$PartType' , '$PartLbl' , '$SKU' , '$Des' , '$Price1' , '$Price2' )");	
	}
