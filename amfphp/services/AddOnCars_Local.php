...
class AddOnCars {
	...
	function getDealPartsInfo($deal_id,$part_type){
		return mysql_query("SELECT 
						   deal_accessories.aid,deal_accessories.deal_id,deal_accessories.part_type,parts.part_id,parts.part_name,parts.part_price,parts.montly_price 
						   FROM 
						   deal_accessories 
						   INNER JOIN parts ON parts.part_id = deal_accessories.part_id 
						   WHERE deal_accessories.part_type='$part_type' AND deal_accessories.deal_id='$deal_id' GROUP BY parts.part_id");
	}
	
	function getDealWheelInfo($deal_id,$part_type){
		return mysql_query("SELECT 
						   deal_accessories.aid,deal_accessories.deal_id,deal_accessories.part_type,wheels.wheel_id,wheels.part_name,wheels.part_price,wheels.montly_price 
						   FROM deal_accessories 
						   INNER JOIN wheels ON wheels.wheel_id = deal_accessories.part_id 
						   WHERE deal_accessories.part_type='$part_type' AND deal_accessories.deal_id='$deal_id' GROUP BY wheels.wheel_id");
	}
	...
	function getPartByID($part_id){
		return mysql_query("SELECT * 
						   FROM part_assets 
						   INNER JOIN part_dimentions ON part_assets.part_id = part_dimentions.part_id 
						   WHERE part_assets.part_id='$part_id' GROUP BY part_assets.part_id");	
	}
   ...
   /* part_sets  */
	...
	function isPartsSetExist($vehicle_id){
		return mysql_num_rows(mysql_query("SELECT * 
										  FROM parts INNER JOIN part_sets ON parts.part_id = part_sets.part_id 
	...
	function getWheelByID($wheel_id){
		return mysql_query("SELECT * 
						   FROM wheel_assets 
						   INNER JOIN wheel_dimentions ON wheel_assets.wheel_id = wheel_dimentions.wheel_id 
						   WHERE wheel_assets.wheel_id='$wheel_id' GROUP BY wheel_assets.wheel_id");	
	}
	...
	function getAllPartCategories( $tid , $vehicle_id ){
		return mysql_query("SELECT 
						   part_categories.part_category_id, part_categories.part_category_name 
						   FROM part_categories 
						   INNER JOIN parts ON part_categories.part_category_id  = parts.part_category_id 
						   WHERE parts.vehicle_id='$vehicle_id' AND part_categories.part_type_id='$tid' GROUP BY parts.part_category_id");	
	}
	...
	function getWheelTypesById2($wid ){
		return mysql_query("SELECT * FROM wheel_types WHERE wheel_type_id ='$wid' GROUP BY wheel_type_id");	
	}
